...
from bot import user_data, DATABASE_URL, bot, BotCommands
...
from bot.helper.utils.other_utils import update_user_ldata
...
async def addSudo(_, message):
    ...
    if id_:
        ...
        msg = "[error] Give ID or Reply To message of whom you want to Promote."
    await sendMessage(message, msg)
...
async def removeSudo(_, message):
    ...
    if id_ and id_ not in user_data or user_data[id_].get('is_sudo'):
        ...
        msg = "[error] Give ID or Reply To message of whom you want to remove from Sudo"
    await sendMessage(message, msg)
